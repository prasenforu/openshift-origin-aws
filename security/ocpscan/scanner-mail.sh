...
if [ "$MAIL" = "y" ] || [ "$MAIL" = "Y" ]; then
	./send-email.py "ALERT: Authentication concern from $SOURCEIP" "$MAILID"
	exit
else 
        exit
fi
...
if [ "$MAIL" = "y" ] || [ "$MAIL" = "Y" ]; then
	./send-email.py "ALERT: Security issue on $RESOURCE ($OBJNAME)" "$MAILID"
	exit
else 
        exit
fi
